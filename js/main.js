...
(function() {
    ...
    function hapticImpact(style = 'medium') {
        // Gunakan HapticFeedback Telegram jika tersedia
    ...
    function setupEventListeners() {
        ...
        if (elements.prizesTags) {
            elements.prizesTags.addEventListener('click', (e) => {
                if (e.target.classList.contains('tag-remove')) {
                    hapticImpact('light'); // Getar ringan saat hapus tag
        ...
        elements.durationTabs.forEach(tab => {
            tab.addEventListener('click', () => {
                hapticImpact('light'); // Getar ringan saat ganti tab
        ...
        if (elements.mediaRemove) {
            elements.mediaRemove.addEventListener('click', () => {
                hapticImpact('medium'); // Getar sedang saat hapus media
                removeMedia();
            });
        }
        ...
        if (elements.captchaToggle) {
            elements.captchaToggle.addEventListener('change', (e) => {
                hapticImpact('soft'); // Getar lembut saat toggle
        ...
        if (elements.cancelBtn) {
            elements.cancelBtn.addEventListener('click', () => {
                hapticImpact('medium'); // Getar sedang saat batal
        ...
        if (elements.selectRequirementsBtn) {
            elements.selectRequirementsBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                hapticImpact('light'); // Getar ringan saat buka panel
        ...
        if (elements.closePanelBtn) {
            elements.closePanelBtn.addEventListener('click', () => {
                hapticImpact('light'); // Getar ringan saat tutup panel
                if (elements.selectPanel) {
                    elements.selectPanel.style.display = 'none';
                }
            });
        ...
        document.querySelectorAll('.option-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                hapticSelection(); // Getar khusus untuk selection change
    ...
    function handlePrizeInput(e) {
        if (e.key === ',' || e.key === 'Enter') {
            e.preventDefault();
            hapticImpact('soft'); // Getar lembut saat tekan enter/koma
            addPrizeFromInput();
        }
    }
    ...
    function addPrizeFromInput() {
        ...
        if (value) {
            hapticImpact('light'); // Getar ringan saat menambah hadiah
    ...
    function handleMediaSelect(e) {
        ...
        if (file) {
            hapticImpact('medium'); // Getar sedang saat pilih file
    ...
    function handleDrop(e) {
        ...
        if (file) {
            hapticImpact('medium'); // Getar sedang saat drop file
            processMediaFile(file);
    ...
    function processMediaFile(file) {
        if (file.size > 20 * 1024 * 1024) {
            hapticNotification('error'); // Getar error kalau file terlalu besar
    ...
    async function handleSubmit(e) {
        ...
        if (prizes.length === 0) {
            hapticNotification('error'); // Getar error kalau validasi gagal
            alert('Minimal 1 hadiah harus diisi!');
            return;
        }
        
        hapticImpact('heavy'); // Getar kuat saat submit
        ...
        try {
            ...
            hapticNotification('error'); // Getar error kalau gagal
            setButtonLoading(false);
    ...
    function showSuccess(giveawayId) {
        ...
        hapticNotification('success'); // Getar sukses
