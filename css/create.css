...
/* ------------------------------------------------------------------
   CHANNEL/GROUP INPUT - DENGAN 3D DAN ELLIPSIS
   ------------------------------------------------------------------ */
...
/* Update style channel tag dengan 3D dan ELLIPSIS */
.channel-tag {
  display: inline-flex;
  align-items: center;
  gap: 8px;
  padding: 8px 12px 8px 8px;
  background: linear-gradient(145deg, rgba(0, 136, 255, 0.2), rgba(0, 68, 136, 0.3));
...
.channel-tag:hover {
  background: linear-gradient(145deg, rgba(0, 136, 255, 0.3), rgba(0, 68, 136, 0.4));
  transform: translateY(-2px);
  box-shadow: 
    0 12px 20px -6px rgba(0, 136, 255, 0.3),
    inset 0 1px 2px rgba(255, 255, 255, 0.2),
    inset 0 -2px 4px rgba(0, 0, 0, 0.3);
}
...
/* Text nama channel dengan ellipsis - TANPA SPASI DI AKHIR */
.channel-name-text {
  white-space: nowrap;
  overflow: hidden;
...
/* Container untuk details (ID, members, username) */
.channel-details {
  display: flex;
  align-items: center;
  gap: 6px;
...
/* Tooltip untuk melihat teks lengkap saat hover - 3D juga */
.channel-name-text[title]:hover::after {
  content: attr(title);
  position: absolute;
  bottom: 100%;
  left: 0;
...
/* Sudah ada di kode Anda, tapi pastikan properti ini: */
.tag-remove {
    cursor: pointer;
    width: 18px;
    height: 18px;
    ...
    position: relative;
    pointer-events: auto !important; /* Pastikan pointer-events auto */
}
...
.format-btn:hover {
  ...
  transform: translateY(-1px);
  color: white;
  box-shadow: 0 8px 12px -4px rgba(0, 136, 255, 0.3);
}
...
/* Preview formatted text di textarea */
.form-textarea {
  font-family: 'Poppins', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
}

/* Style untuk teks yang diformat */
